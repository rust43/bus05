...
const selectTransportFeature = function (imei) {
  let features = transportVectorSource.getFeatures();
  for (let i = 0; i < features.length; i++) {
    if (features[i].get('imei') === imei) {
      features[i].set('selected', true);
      let deselected = [];
      if (mapSelectInteraction.getFeatures().getLength() > 0) deselected = [mapSelectInteraction.getFeatures().item(0)];
